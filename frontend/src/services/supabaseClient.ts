...
if (!supabaseUrl || !supabaseAnonKey) {
  throw new Error(
    "VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY são obrigatórias para o Realtime funcionar"
  );
}
...
try {
  ...
  throw new Error(
    "VITE_SUPABASE_URL deve ser uma URL válida (ex: https://seu-projeto.supabase.co)"
  );
}
    ...
    try {
      const channel = supabase
        ...
        .subscribe((status, err) => {
          if (err) {
            console.error(
              `❌ Erro na subscription para cliente ${clientId}:`,
              err
            );
          }
          ...
          if (status === "CHANNEL_ERROR") {
            ...
            console.error(
              `⏰ Timeout na subscription para cliente ${clientId}`
            );
          }
        });
      ...
      console.error(
        `❌ Erro ao criar subscription para cliente ${clientId}:`,
        error
      );
    }
  }
